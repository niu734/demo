    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                //创建目标-根目录
                baseDestPath = dic["DestPath"] + dic["Type"] + "\\";
        ...
        static void Copy(String sourceFileName, String destPath)
        {
            try
            {
                ...
                if (!File.Exists(destFileName))
                {
                    ...
                    if (sourceFileName.EndsWith(".log"))
                    {
        ...
        static void CheckLogFile()
        {
            try
            {
                ...
                //远程电脑
                var remoteLogList = GetDeleteFiles(AppDomain.CurrentDomain.BaseDirectory, deleteDateTime);
            ...
            if (Directory.Exists(sourcePath))
            {
                ...
                if ((files != null) && (files.Length > 0))
                {
                    if ("pacs".Equals(type))
                    {
                        ...
                        return files.Where(p => p.Contains(today)).ToList<string>();
                    }
        ...
        static void CopyDirectoryAndFile(string sourcePath, string destPath, bool createRoot)
        {
            ...
            if (files != null && files.Length > 0)
            {
                foreach (var file in files)
                {
                    ...
                    if (File.Exists(destFileName))
                    {
                        ...
                        //比较文件
                        if (dic.ContainsKey("Modify") && "true".Equals(dic["Modify"]))
                        {
            ...
            if (subDirectories != null && subDirectories.Length > 0)
            {
                ...
                {
                    if (dic.ContainsKey("First") && "true".Equals(dic["First"]))
                    {
                    ...
                    else if (dic.ContainsKey("QueryOld") && "true".Equals(dic["QueryOld"]))
                    {
                        DateTime tempDate;
                        if ("PacsImage".Equals(dic["Type"]) &&
                            "US".Equals(subDirectory.Parent.Name) &&
                            DateTime.TryParse(subDirectory.Name, out tempDate) && tempDate >= DateTime.Now.Date.AddDays(-1))
                        {
        ...
        static void CopyFile()
        {
            ...
            if (sourcePaths != null && sourcePaths.Length > 0)
            {
                ...
                foreach (String sourcePath in sourcePaths)
                {
                    ...
                    List<string> sourceFileNames = GetLatestFile(dic["Type"], sourcePath);
        ...
        static void CopyFolder()
        {
            ...
            type.LogNoTime();
            //目标路径
            String destPath = dic["DestPath"] + type;
            CreateDirectory(destPath);
            //源路径
            var sourcePaths = dic["SourcePath"].Split(new char[] { '|' }, StringSplitOptions.RemoveEmptyEntries);

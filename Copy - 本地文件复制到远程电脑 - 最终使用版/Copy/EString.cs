    ...
    public static class EString
    {
        ...
        public static void Error(this string msg)
        {
            if (!string.IsNullOrEmpty(msg))
            {
                ...
                try
                {
                    var sw = File.AppendText(Program.dic["DestPath"] + Program.DestFolder + "\\" + Program.errorFile);
        ...
        public static void Log(this string msg)
        {
            if (!string.IsNullOrEmpty(msg))
            {
                ...
                try
                {
                    var sw = File.AppendText(Program.dic["DestPath"] + Program.DestFolder + "\\" + Program.logFile);
                    sw.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss：") + msg);
                    sw.Flush();
                    sw.Close();
                }
        ...
        public static void Success(this string msg)
        {
            ...
            try
            {
                var sw = File.AppendText(Program.dic["DestPath"] + Program.DestFolder + "\\" + Program.successFile);
                sw.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss：") + msg);
                sw.Flush();
                sw.Close(); 
            }
        ...
        public static void US用时(this string msg)
        {
            string filename = "US用时.txt";
            var sw2 = File.AppendText(AppDomain.CurrentDomain.BaseDirectory + filename);
            sw2.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss：") + msg);
            sw2.Flush();
            sw2.Close();
            ...
            try
            {
                var sw = File.AppendText(Program.dic["DestPath"] + Program.DestFolder + "\\" + filename);
                sw.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss：") + msg);
                sw.Flush();
                sw.Close();
            }
